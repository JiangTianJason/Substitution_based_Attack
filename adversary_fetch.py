...
def attack_process(idx, args, q):
    ...
    attack_models = {
        ...
        'dwb': OpenAttack.attackers.DeepWordBugAttacker,
        "tb": OpenAttack.attackers.TextBuggerAttacker,
    }
    if args.adversary not in ['uat','dwb','tb']:
        attacker = attack_models[args.adversary](skip_words=skip_words)
    else:
        attacker = attack_models[args.adversary]()

    # Build evaluation object
    options = {"success_rate": True, "fluency": False, "mistake": False, "semantic": False, "levenstein": False,
               "word_distance": False, "modification_rate": False, "running_time": True, "progress_bar": True,
    ...
    while True:
        ...
        if len(adv_data) > 0:
            ...
            if sample_list:
                # dict3 = Merge({'index': data_idx,'adversary_samples': sample_list,'predictions': prediction_list},attack_eval.measure(" ".join(data2sample(data,id2rel)["token"])," ".join(adv_sample["token"])))
                dict3 = {'index': data_idx,'adversary_samples': sample_list,'predictions': prediction_list}

                with open(args.output_file, 'a') as f:
                    f.write(json.dumps(dict3,ensure_ascii=False, default=default_dump) + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--adversary', '-a', default='hf',
                        choices=['pw', 'tf', 'hf', 'uat', 'gen','dwb','tb'])
    ...
    for data_type in DATA_TYPES[args.dataset]:
        ...
        # args.output_file = r"/content/drive/MyDrive/output/{}_{}_{}.txt".format(args.dataset,args.model,args.adversary)
